...
# load data
#rawdata=read.csv("COP_Features_CF_raw.csv",header=T)
#rawdata=read.csv("COP_Features_CR_raw_ALL.csv",header=T)
#rawdata=read.csv("COP_Features_OR_raw.csv",header=T)
rawdata=read.csv("COP_Features_OF_raw.csv",header=T)
#rawdata=read.csv("COP_Features_C4_raw.csv",header=T)
F_index=c(2,3);  #CF#for single,replace 3¡A3 to...72)
#F_index=c(2,3); #CR#for single,replace 3¡A3 to...72)
#F_index=c(2,3); #OF#for single,replace 3¡A3 to...72)
#F_index=c(2,3); #OR#for single,replace 3¡A3 to...72)
#F_index=c(2,3); #C4#for single,replace 3¡A3 to...72)
dim(rawdata)
# data separation
np=45 # 20% testing data#The np value can be changed according to requirements #C4's np value is 177
...
np=45 # 20%
	...
	for (i in 1:20)#See how many times it needs to run, this time it¡¦s 20 runs
...
print(np45.sd) 
Result_all
 write.table(Result_all,file="cf_6",sep=",",col.names=FALSE,row.names=FALSE,quote=TRUE) # edit files names#Just change cf_6
